...
class ModelCatMissingHandler:
    def __init__(self, books_missing_genre, ratings_df):
        self.books_missing_genre = books_missing_genre
        self.ratings_df = ratings_df
        self.nn_model, self.tfidf_matrix, self.svd_model = self.get_nn_output(books_missing_genre=books_missing_genre)

    def get_nn_output(self, books_missing_genre):
        ...
        books_missing_genre['content_features'] = (
                books_missing_genre['book_author'] + " " +
                books_missing_genre['publisher'] + " " +
                books_missing_genre['year_of_publication'].astype(str)
        )

        # Apply TF-IDF and fit Nearest Neighbors
        tfidf = TfidfVectorizer()
        tfidf_matrix = tfidf.fit_transform(books_missing_genre['content_features'])

        nn_model = NearestNeighbors(metric='cosine', algorithm='brute')
        nn_model.fit(tfidf_matrix)
        print('Model 2 return')
